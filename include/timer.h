...
{
    TimerFlag = 1;
    myMillis++; // Increment the millisecond counter
}
...
{
    // AVR timer/counter controller register TCCR1
    TCCR1A = 0x00; // Set to CTC mode with OCR1A as the top
    TCCR1B = 0x0C; // bit3 = 1: CTC mode (clear timer on compare)
                   // bit2bit1bit0=011: prescaler /64
                   // SO, 16 MHz clock or 16,000,000 /64 = 250,000 ticks/s
...
{
    // CPU automatically calls when TCNT1 == OCR1A (every 1 ms per TimerOn settings)
    _avr_timer_cntcurr--; // Count down to 0 rather than up to TOP
    if (_avr_timer_cntcurr == 0)
    {               // results in a more efficient compare
        TimerISR(); // Call the ISR that the user uses
